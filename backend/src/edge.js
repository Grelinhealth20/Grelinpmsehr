...
const WAF_SIGNATURES = [
  { name: 'sqli', re: /(\bunion\b[\s(]+\bselect\b)|(\bselect\b\s+\*\s+\bfrom\b)|(\binsert\b\s+\binto\b)|(\bdrop\b\s+\btable\b)|(\b(or|and)\b\s+\d+\s*=\s*\d+)|(\b(or|and)\b\s+(['"])[^'"]{0,20}\7\s*=\s*\7)|(--\s)|(\/\*[\s\S]{0,200}?\*\/)|(\bsleep\s*\()|(\bbenchmark\s*\()|(\bwaitfor\b\s+\bdelay\b)|(\binformation_schema\b)/i },
  ...
  { name: 'rce', re: /(?:[;|`]|\$\()\s*(?:whoami|wget|nc|bash|powershell)\b|(?:[;|`]|\$\()\s*curl\s+(?:-|['"]|https?:|ftp|\/|\w+:\/\/)|(?:[;|`]|\$\()\s*(?:cat|ls|id|sh|ping)(?=\s*(?:[-\/;|&<>]|$))|(\bcmd\.exe\b)|(\/bin\/(ba)?sh\b)|(\$\{jndi:)/i },
];
...
export function waf(req, res, next) {
  ...
  try { decodedUrl = decodeURIComponent(req.originalUrl); }
  ...
  for (const v of Object.values(req.query || {})) haystacks.push(Array.isArray(v) ? v.join(' ') : String(v));
